    ...
    ThreadedAssignment(message)
{
    ...
    packetReceiver.registerListener(PacketType::ChallengeOwnership, this, "handleChallengeOwnershipDataPacket");
...
void AvatarMixer::handleAvatarIdentityPacket(QSharedPointer<ReceivedMessage> message, SharedNodePointer senderNode) {
    ...
    if (senderNode->getLinkedData()) {
        ...
        if (nodeData != nullptr) {
            ...
            avatar.setIsServerData(true);

            // parse the identity packet and update the change timestamp if appropriate
            bool identityChanged = false;
            bool displayNameChanged = false;
            bool skeletonModelUrlChanged = false;
            avatar.processAvatarIdentity(message->getMessage(), identityChanged, displayNameChanged, skeletonModelUrlChanged, senderNode);
